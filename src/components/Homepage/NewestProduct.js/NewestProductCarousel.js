...
import NewestProductCarouselSkeleton from './NewestProductCarouselSkeleton';
...
const useStyles = makeStyles((theme) => ({
  ...
  carousel: {
    ...
    "& .carousel-control-prev-icon": {
      backgroundImage: `url(${Constants.PREV_ICON_IMG})`,
    },
    "& .carousel-control-next-icon": {
      backgroundImage: `url(${Constants.NEXT_ICON_IMG})`,
    },
    '& .carousel-indicators ': {
      bottom: -40,
    },
...
export default function NewestProductCarousel() {
  ...
  useEffect(() => {
    const fetchNewestProduct = async () => {
      try {
        const res = await axios.get(process.env.REACT_APP_REMOTE_URL + '/newest-products');
